...
class MultiScaleEncoder2D(nn.Module):
    """
    Multi-scale encoder for 2D complex fields (images/lattices).
    """
    features: Sequence[int] = (64, 128, 256)
    num_heads: int = 8
    dropout_rate: float = 0.1
    ...
    def __call__(
        ...
        for i, features in enumerate(self.features):
            ...
            # Simple 3x3 conv for local feature extraction at each scale
            h = ComplexConv(features=features, kernel_size=(3, 3))(h)
            h = ComplexLayerNorm()(h)
            ...
            # Downsample for next level (if not last)
            if i < len(self.features) - 1:
                # 2x2 Average pooling in complex space
                # h shape: (B, H, W, C)
                B, H, W, C = h.shape
                # Ensure H, W are even
                h_real = jnp.real(h).reshape(B, H // 2, 2, W // 2, 2, C).mean(axis=(2, 4))
                h_imag = jnp.imag(h).reshape(B, H // 2, 2, W // 2, 2, C).mean(axis=(2, 4))
                h = h_real + 1j * h_imag
        
        return embeddings


class MultiScaleDecoder2D(nn.Module):
    """
    Multi-scale decoder for 2D complex fields.
    """
    features: Sequence[int] = (256, 128, 64)
    num_heads: int = 8
    dropout_rate: float = 0.1
    ...
    def __call__(
        ...
        # Apply 2D UNet at the coarsest level or across a bottleneck
        # Here we use the ComplexUNet2D as a hierarchical processor
        # We process the combined embeddings
        
        # Process with ComplexUNet2D (reduced size for efficiency)
        decoder_unet = ComplexUNet2D(
            features=self.features[0] // 2,
            num_blocks=2,
            dropout_rate=self.dropout_rate
        )
        
        # In a real hierarchical model, we might upsample and merge
        # For simplicity, we decode the last embedding and project
        h = decoder_unet(h, t, context, abs_phase, action, deterministic=deterministic)
        
        return h


class MultiScaleSIFEUNet2D(nn.Module):
    """
    Full multi-scale 2D architecture for visual cognitive modeling.
    """
    features: Sequence[int] = (64, 128, 256)
    output_features: int = 3  # RGB channels
    num_heads: int = 8
    dropout_rate: float = 0.1
    ...
    def __call__(
        ...
        # Embed time
        t_emb = ComplexTimeEmbedding(dim=self.features[0])(t)
        
        # Embed action if present
        action_emb = None
        if action is not None:
            action_emb = ComplexTimeEmbedding(dim=self.features[0])(action)
        ...
        # Decoder
        decoder = MultiScaleDecoder2D(
            features=self.features[::-1],
            num_heads=self.num_heads,
            dropout_rate=self.dropout_rate
        )
        h = decoder(embeddings, t, context, abs_phase, action, deterministic)
        
        # Resize back to original sequence if decoder didn't reach it
        if h.shape[1:3] != x.shape[1:3]:
            h_r, h_i = jnp.real(h), jnp.imag(h)
            h_r = jax.image.resize(h_r, (x.shape[0], x.shape[1], x.shape[2], h.shape[3]), method='bilinear')
            h_i = jax.image.resize(h_i, (x.shape[0], x.shape[1], x.shape[2], h.shape[3]), method='bilinear')
            h = h_r + 1j * h_i
